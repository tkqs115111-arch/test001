...
//  PART 1: 設定與 API
// =========================================================
// 【在此手動修改資料庫版本日期】讓使用者知道目前資料的時效
const DB_VERSION_DATE = '2025-12-31'; 
...
async function initData() {
    ...
    if(detectedOS.size > 0) {
        OS_LIST = Array.from(detectedOS)
            .filter(os => !IGNORED_OS.includes(os))
...
function updateLastSyncDate() {
    const el = document.getElementById('db-update-date');
    if(el) {
        el.innerHTML = `<i class="far fa-clock"></i> 資料庫版本: ${DB_VERSION_DATE}`;
    }
}
...
function renderProducts(data, viewType) {
    ...
    data.forEach((product) => {
        ...
        let shortOS = targetOS
            .replace("Windows Server", "Win Srv")
            .replace("Datacenter", "")
            .replace("Standard", "")
            .trim();
            ...
            <div class="row-main-content">
                ...
                <div class="row-body">
                    <div class="data-group">
                        ...
                        <div class="data-item">
                            <span class="data-label" style="color: #d93025; font-weight: 800;" title="${targetOS}">
                                Driver Version <span style="font-size:0.9em;">(${shortOS})</span>
                            </span>
